---
name: code-implementing-features
description: Implement features and documentation changes through analysis, scoped edits, tests, and completion checks. Use when developing new functionality, changing code, updating documentation, or when the user is ready to start coding a feature.
---
...
Before creating or modifying Python files, read and follow [Python import standards](python-style.md).
...
Planning artifacts under `docs/plans/` are local-only by default and are not product documentation. Do not stage, commit, push, or include them in a pull request unless the user explicitly asks to publish a specific plan.
