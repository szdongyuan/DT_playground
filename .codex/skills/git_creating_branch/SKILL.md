---
name: git-creating-branch
description: Create local personal or task branches from the latest remote code with consistent naming and optional issue links. Use when starting feature work, bug fixes, refactors, experiments, or any development that needs a fresh branch.
---
...
Before `git fetch` or any `gh` command that accesses GitHub, hydrate proxy and
token variables from persisted User/Machine environment values. Codex shell
processes may run as `codexsandboxoffline` and may not inherit `HTTP_PROXY`,
`HTTPS_PROXY`, `ALL_PROXY`, or `GH_TOKEN` into the current process.
