---
name: git-creating-pr
description: Create pull requests from personal branches to target branches with complete title, description, and validation context. Use when submitting code for review, opening a PR, creating a pull request, or preparing changes for merge.
---
...
### GitHub CLI Environment on Windows Codex Desktop

Before any `gh` command, hydrate proxy and token variables from persisted
User/Machine environment values. Codex shell processes may run as
`codexsandboxoffline` and may not inherit `HTTP_PROXY`, `HTTPS_PROXY`,
`ALL_PROXY`, or `GH_TOKEN` into the current process.
