...
description: Commit and push changes safely to remote personal branches while avoiding protected main or master branches. Use when committing code changes, staging files, pushing to remote, syncing local work, or preparing branch updates.
---
...
## GitHub Remote Environment on Windows Codex Desktop

Before `git fetch`, `git push`, or any `gh` command that accesses GitHub, hydrate
proxy and token variables from persisted User/Machine environment values. Codex
shell processes may run as `codexsandboxoffline` and may not inherit
`HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, or `GH_TOKEN` into the current process.
...
Do not print `GH_TOKEN`; use `[bool]$env:GH_TOKEN` for diagnostics.
...
- Credentials/secrets
- Local configuration
- `docs/plans/` local planning artifacts, unless the user explicitly requests publishing a specific plan

Before committing, verify `git status --short` and `git diff --cached --name-only` do not contain `docs/plans/` by default.
