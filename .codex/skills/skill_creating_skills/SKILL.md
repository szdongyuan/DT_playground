---
name: skill-creating-skills
description: Create and organize project skills with consistent frontmatter, naming, structure, triggers, and documentation. Use when creating skills, writing SKILL.md files, updating skill metadata, authoring descriptions, organizing skills, or validating skill conventions.
---
...
|--------|--------|----------|
| `git-` | Git operations | `git-commit-messages`, `git-submitting-issues`, `git-branch-naming` |
| `code-` | Code operations | `code-review`, `code-formatting`, `code-refactoring` |
| `doc-` | Documentation | `doc-api-reference`, `doc-changelog`, `doc-readme` |
| `test-` | Testing | `test-unit-setup`, `test-integration`, `test-e2e` |
| `deploy-` | Deployment | `deploy-docker`, `deploy-kubernetes`, `deploy-ci-cd` |
| `db-` | Database | `db-migration`, `db-backup`, `db-schema-design` |
| `api-` | API related | `api-rest-design`, `api-graphql`, `api-authentication` |
| `util-` | Utilities | `util-file-processing`, `util-data-conversion` |
| `skill-` | Skill management | `skill-creating-skills`, `skill-organizing` |
| `project-` | Project setup | `project-structure`, `project-initialization` |
| `debug-` | Debugging | `debug-logging`, `debug-profiling` |
| `security-` | Security | `security-auth`, `security-encryption` |

### Naming Examples

**Good names:**
```
git-commit-messages       # Git + action + target
code-review               # Code + action
test-unit-setup           # Test + type + action
api-rest-design           # API + protocol + action
deploy-docker-compose     # Deploy + tool + detail
```

**Bad names:**
```
helper                    # No category prefix
my_skill                  # Uses underscores instead of hyphens
Code-Review               # Uses uppercase
...
```
.codex/skills/{category}-{action}-{target}/
...
| Type | Path | Scope |
|------|------|-------|
| Project | `.codex/skills/skill-name/` | Shared with repository |
| Personal | `~/.codex/skills/skill-name/` | Available across all projects |
...
### Naming
- [ ] Uses category prefix (e.g., `git-`, `code-`, `test-`)
...
2. Create folder: `.codex/skills/{category}-{action}-{target}/`
