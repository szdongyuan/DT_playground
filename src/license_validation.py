...
def get_license_failure_text() -> tuple[str, str]:
    """Return the translated license failure dialog title and message."""
    return tr_("Authorization failed"), tr_("Software authorization failed")
...
def validate_license(app_root: str | Path) -> bool:
    ...
    try:
        stored_hash = (
            (Path(app_root) / "salt").read_text(encoding="utf-8").strip().lower()
        )
