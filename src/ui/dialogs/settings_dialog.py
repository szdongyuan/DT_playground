...
class SettingsDialog(QDialog):
    ...
    def _init_ui(self):
        """初始化界面"""
        self.setWindowTitle(tr_("Settings"))
        ...
        # 音频设置
        self.tabs.addTab(self._create_audio_tab(), tr_("Audio"))
        
        # 模型设置
        self.tabs.addTab(self._create_model_tab(), tr_("Model"))
        
        # 界面设置
        self.tabs.addTab(self._create_ui_tab(), tr_("UI"))
        
        # GPU设置
        self.tabs.addTab(self._create_gpu_tab(), tr_("GPU"))
        ...
        self.reset_btn = QPushButton(tr_("Reset to defaults"))
        ...
        self.save_btn = QPushButton(tr_("Save"))
    ...
    def _create_audio_tab(self) -> QWidget:
        ...
        # 采样设置
        sample_group = QGroupBox(tr_("Sampling"))
        ...
        sample_layout.addRow(tr_("Sample rate (Hz):"), self.sample_rate_combo)
        
        self.mono_check = QCheckBox(tr_("Convert to mono"))
        ...
        # 频谱分析设置
        spectrum_group = QGroupBox(tr_("Spectrum analysis"))
        ...
        spectrum_layout.addRow(tr_("FFT window size:"), self.n_fft_combo)
        ...
        spectrum_layout.addRow(tr_("Hop length:"), self.hop_length_spin)
        
        self.n_mels_spin = QSpinBox()
        self.n_mels_spin.setRange(40, 256)
        self.n_mels_spin.setValue(128)
        spectrum_layout.addRow(tr_("Mel filters:"), self.n_mels_spin)
        
        self.n_mfcc_spin = QSpinBox()
        self.n_mfcc_spin.setRange(10, 40)
        self.n_mfcc_spin.setValue(20)
        spectrum_layout.addRow(tr_("MFCC coefficients:"), self.n_mfcc_spin)
        
        layout.addWidget(spectrum_group)
        
        # 预处理设置
        preprocess_group = QGroupBox(tr_("Preprocessing"))
        preprocess_layout = QFormLayout(preprocess_group)
        
        self.normalize_check = QCheckBox(tr_("Normalize audio"))
        self.normalize_check.setChecked(True)
        preprocess_layout.addRow("", self.normalize_check)
        
        self.remove_silence_check = QCheckBox(tr_("Remove silence"))
        ...
        preprocess_layout.addRow(tr_("Silence threshold:"), self.silence_threshold_spin)
    ...
    def _create_model_tab(self) -> QWidget:
        ...
        # 默认训练参数
        train_group = QGroupBox(tr_("Default training parameters"))
        ...
        train_layout.addRow(tr_("Learning rate:"), self.default_lr_spin)
        
        layout.addWidget(train_group)
        
        # 模型保存设置
        save_group = QGroupBox(tr_("Model saving"))
        save_layout = QFormLayout(save_group)
        
        model_path_layout = QHBoxLayout()
        self.model_save_path = QLineEdit()
        self.model_save_path.setPlaceholderText(tr_("Choose a model save directory..."))
        ...
        save_layout.addRow(tr_("Save path:"), model_path_layout)
        
        self.auto_save_check = QCheckBox(tr_("Auto-save best model"))
        self.auto_save_check.setChecked(True)
        save_layout.addRow("", self.auto_save_check)
        
        self.save_format_combo = QComboBox()
        self.save_format_combo.addItems(["SavedModel", "H5", "Keras"])
        save_layout.addRow(tr_("Save format:"), self.save_format_combo)
        
        layout.addWidget(save_group)
        
        # 早停设置
        early_stop_group = QGroupBox(tr_("Early stopping"))
        early_stop_layout = QFormLayout(early_stop_group)
        
        self.early_stop_check = QCheckBox(tr_("Enable early stopping"))
        ...
        early_stop_layout.addRow(tr_("Patience:"), self.patience_spin)
        ...
        early_stop_layout.addRow(tr_("Minimum delta:"), self.min_delta_spin)
    ...
    def _create_ui_tab(self) -> QWidget:
        ...
        # 主题设置
        theme_group = QGroupBox(tr_("Theme"))
        theme_layout = QFormLayout(theme_group)
        
        self.theme_combo = QComboBox()
        self.theme_combo.addItems([tr_("Dark"), tr_("Light"), tr_("Follow system")])
        theme_layout.addRow(tr_("Theme:"), self.theme_combo)

        # Language (takes effect after restart)
        self.language_combo = QComboBox()
        self.language_combo.addItem(tr_("Chinese (Simplified)"), "zh_CN")
        self.language_combo.addItem(tr_("English"), "en_US")
        theme_layout.addRow(tr_("Language:"), self.language_combo)
        
        layout.addWidget(theme_group)
        
        # 可视化设置
        viz_group = QGroupBox(tr_("Visualization"))
        viz_layout = QFormLayout(viz_group)
        
        self.waveform_color = QComboBox()
        self.waveform_color.addItems([tr_("Blue"), tr_("Green"), tr_("Purple"), tr_("Orange")])
        viz_layout.addRow(tr_("Waveform color:"), self.waveform_color)
        
        self.spectrogram_cmap = QComboBox()
        self.spectrogram_cmap.addItems(["viridis", "magma", "plasma", "inferno", "jet"])
        viz_layout.addRow(tr_("Spectrogram colormap:"), self.spectrogram_cmap)
        
        self.show_grid_check = QCheckBox(tr_("Show grid"))
        ...
        # 更新频率
        update_group = QGroupBox(tr_("Update"))
        ...
        update_layout.addRow(tr_("Chart update interval:"), self.chart_update_spin)
    ...
    def _create_gpu_tab(self) -> QWidget:
        ...
        # GPU设置
        gpu_group = QGroupBox(tr_("GPU settings"))
        gpu_layout = QFormLayout(gpu_group)
        
        self.use_gpu_check = QCheckBox(tr_("Use GPU acceleration"))
        ...
        self.gpu_memory_spin.setSuffix(tr_(" % (0=auto)"))
        gpu_layout.addRow(tr_("GPU memory limit:"), self.gpu_memory_spin)
        
        self.mixed_precision_check = QCheckBox(tr_("Mixed precision training"))
        ...
        # GPU信息
        info_group = QGroupBox(tr_("GPU info"))
        info_layout = QVBoxLayout(info_group)
        
        self.gpu_info_label = QLabel(tr_("Detecting GPU..."))
        self.gpu_info_label.setWordWrap(True)
        info_layout.addWidget(self.gpu_info_label)
        
        detect_btn = QPushButton(tr_("Re-detect"))
    ...
    def _browse_model_path(self):
        """浏览模型保存路径"""
        path = QFileDialog.getExistingDirectory(self, tr_("Select a model save directory"))
        if path:
            self.model_save_path.setText(path)
    
    def _load_settings(self):
        """加载当前设置"""
        # Prefer caller-provided settings, fallback to ConfigManager for language.
        settings = self.current_settings or {}
        if not settings:
            try:
                from src.utils.config import config

                settings = {"ui.language": config.get("ui.language", "zh_CN")}
            except Exception:
                settings = {}

        # Language
        lang = settings.get("ui.language") or settings.get("language")
        if lang and hasattr(self, "language_combo"):
