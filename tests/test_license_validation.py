...
def test_license_failure_text_uses_translatable_messages(monkeypatch):
    monkeypatch.setattr(license_validation, "tr_", lambda message: f"tr:{message}")

    assert license_validation.get_license_failure_text() == (
        "tr:Authorization failed",
        "tr:Software authorization failed",
    )
